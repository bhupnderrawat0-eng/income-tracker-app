...
# ========================= LOANS =========================
    ...
    loan = loans_df[loans_df["display"] == selected].iloc[0]
    ...
    if st.button("Add Payment"):
        c.execute(
            ...
            (loan["name"], pay_date.strftime("%Y-%m-%d"), pay_amt)
        )
        conn.commit()
    ...
    cust_payments = payments_df[payments_df["name"] == loan["name"]].sort_values("date")
